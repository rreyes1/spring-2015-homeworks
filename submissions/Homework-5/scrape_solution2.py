...
def get_city_page(city, state):
    ...
    with open(os.path.join(“data/“, city + '-tourism-page.html'), "w") as h:
        h.write(html)
...
def get_hotellist_page(city_url, page_count):
    ...
    html = response.text.encode('utf-8')
    # Save the webpage
    with open(os.path.join(“data\”, ‘boston’ + '-hotelist-' + str(page_count) + '.html'), "w") as h:
        h.write(html)
    return html
...
if __name__ == "__main__":
 ...
 #   city_url = get_city_page('boston', "massachusetts")
    city_url = get_city_page(Boston,Massachusetts)
    c=0
    while(True):
        c +=1
